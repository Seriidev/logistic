...
export default function LCLForm({ formData, onChange, onNext }) {
  ...
  return (
    <StepWrapper
      eyebrow={t("services.lcl.eyebrow", { ns: "seaCargoBooking" })}
      title={t("steps.shipmentDetails", { ns: "booking" })}
      description={t("services.lcl.description", { ns: "seaCargoBooking", time: deliveryTime })}
    >
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 max-w-5xl mx-auto items-start">
        ...
        >
          ...
          {/* Shipment */}
          <h3 className="text-sm font-bold text-gray-900 mb-3 uppercase tracking-wide">{t("form.sections.shipment", { ns: "booking" })}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 mb-6">
            <div className="min-w-0">
              <label htmlFor="totalUnits" className={labelClass}>{t("form.fields.totalUnits", { ns: "booking" })}</label>
              <input id="totalUnits" type="number" min="1" placeholder={t("form.placeholders.totalUnits", { ns: "booking" })}
            ...
            <div className="min-w-0">
              <label htmlFor="commodityType" className={labelClass}>{t("form.fields.commodityType", { ns: "booking" })}</label>
              <select id="commodityType" value={formData.commodityType || ""} onChange={update("commodityType")}
                className={`${inputClass} cursor-pointer`}>
                <option value="" disabled>{t("form.placeholders.selectCommodity", { ns: "booking" })}</option>
                {COMMODITY_TYPES.map((c) => (
                  <option key={c} value={c}>{t(`commodityTypes.${c}`, { ns: "seaCargoBooking" })}</option>
                ))}
              </select>
            </div>
            <div className="min-w-0">
