...
const COUNTRY_KEYS = [
  ...
  "spain", "mexico", "southKorea", "russia", "turkey",
];
...
export default function LocHero() {
  ...
  const continents = useMemo(
    () => CONTINENT_KEYS.map((key) => ({ key, label: t(`continents.${key}`) })),
    [t],
  );

  const countries = useMemo(
    () => COUNTRY_KEYS.map((key) => ({ key, label: t(`countries.${key}`) })),
    [t],
  );
  ...
  return (
    <div className="page-container py-4 sm:py-6 min-w-0">
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-4 flex-wrap">
        <a href="/" className="hover:text-blue-500 transition-colors no-underline text-gray-500">
      ...
      <div className="relative rounded-2xl sm:rounded-3xl bg-blue-500 overflow-hidden px-5 py-8 sm:px-10 sm:py-10 min-h-[200px] flex flex-col justify-center">
        <div className="hidden md:block absolute top-4 right-[40%] lg:right-[280px] opacity-80 pointer-events-none">
          <img src="/box-decor.png" alt="" className="w-12 sm:w-16" onError={(e) => { e.target.style.display = "none"; }} />
        </div>
        <div className="hidden md:block absolute bottom-4 left-[40%] lg:left-[320px] opacity-60 pointer-events-none">
          <img src="/box-decor2.png" alt="" className="w-10" onError={(e) => { e.target.style.display = "none"; }} />
        </div>
        ...
        <div className="hidden md:block absolute right-20 lg:right-[160px] top-1/2 -translate-y-1/2 w-32 h-32 lg:w-[200px] lg:h-[200px] rounded-full border-[24px] lg:border-[40px] border-white/10 pointer-events-none" />

        <div className="relative z-10 w-full max-w-full lg:max-w-[500px] min-w-0 pr-0 sm:pr-24 md:pr-32">
          <h1 className="text-xl sm:text-2xl font-extrabold text-white mb-4 leading-tight">{t("hero.title")}</h1>
          ...
          <p className="text-blue-100 text-xs mb-2 font-medium">{t("hero.continentLabel")}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 min-w-0">
            <div className="relative w-full sm:w-auto">
              <select
                ...
                className="w-full sm:min-w-[160px] h-11 pl-4 pr-8 rounded-full bg-white text-gray-900 text-sm font-medium border-none outline-none cursor-pointer appearance-none font-[inherit]"
              >
                {continents.map((c) => (
            ...
            <div className="relative flex-1 min-w-0 w-full">
              ...
              {showResults && search && filtered.length > 0 && (
                <div className="absolute top-[calc(100%+6px)] left-0 w-full bg-white rounded-2xl shadow-lg overflow-hidden z-50 border border-gray-100 max-h-48 overflow-y-auto">
                  {filtered.map((country) => (
                    <div
                      key={country.key}
                      onMouseDown={() => { setSearch(country.label); setShowResults(false); }}
                      className="px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-500 cursor-pointer transition-colors duration-100"
                    >
                      {country.label}
                    </div>
                  ))}
